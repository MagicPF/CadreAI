...
def process_pdb(row, row_num, input_csv, output_dir, cache_dir, locks_dir,
    ...
    if not pdb_id.isalnum() or len(pdb_id) != 4:
        print(f"Row {row_num}: 无效的PDB ID格式，已跳过")
        return

    # 分布式锁：尝试创建lock文件，若已存在则跳过
...
if __name__ == "__main__":
    ...
    api_key = "EMPTY"
    input_csv = "./datasets/druggen/pdb_ids.csv"
    output_dir = f"output_druggen_{commander_model.replace(':', '_').replace('-', '_')}_MT_with_insight_opt"
